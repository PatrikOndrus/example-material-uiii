...
function iteriaLowcode(options) {
  var _a, _b
  const injectLowcode =
    (_a = options.injectLowcode) != null ? _a : options.mode != 'production'
  if (injectLowcode)
  ...
  const iteriaScript = __spreadProps(
    __spreadValues(
      __spreadValues(
        {
          fsPort: 3e3,
  ...
  if (options.version)
    ideDevserverPath = `'/ide-devserver.es.js'` // `'https://unpkg.com/@iteria-app/ide-devserver@${options.version}/dist/ide-devserver.es.js'`;
  else ideDevserverPath = `'/ide-devserver.es.js'` // `'https://unpkg.com/@iteria-app/ide-devserver/dist/ide-devserver.es.js'`;
  const injectDevServer =
    (options == null ? void 0 : options.version) === 'workspace:*'
      ? false
      : true
  const metas = []
  const links = [
    ...
    config: (config) => {
      ...
      return __spreadProps(__spreadValues({}, config), {
        server: __spreadProps(__spreadValues({}, config.server), {
          hmr,
          fs: __spreadProps(
            __spreadValues({}, (_b2 = config.server) == null ? void 0 : _b2.fs),
            {
              strict: false
            }
          )
